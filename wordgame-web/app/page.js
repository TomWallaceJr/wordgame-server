...
function Home() {
    ...
    const [lobbyId, setLobbyId] = (0, react_1.useState)("lobby-1");
    const handleJoin = () => {
    ...
    return (<main className="min-h-screen bg-slate-950 text-white flex items-center justify-center px-4">
      <div className="w-full max-w-xl space-y-6 rounded-2xl bg-slate-900 border border-slate-800 p-8 shadow-xl">
        ...
        <div className="space-y-3">
          ...
          <input className="w-full rounded-lg bg-slate-800 px-3 py-2 border border-slate-700 outline-none focus:border-emerald-500 transition" placeholder="Your name" value={name} onChange={(e) => setName(e.target.value)}/>
        </div>

        <div className="space-y-3">
          <label className="text-sm text-slate-300">Choose a lobby</label>
          <select className="w-full rounded-lg bg-slate-800 px-3 py-2 border border-slate-700 outline-none focus:border-emerald-500 transition" value={lobbyId} onChange={(e) => setLobbyId(e.target.value)}>
            <option value="lobby-1">Lobby 1</option>
            <option value="lobby-2">Lobby 2</option>
          </select>
        </div>

        <button onClick={handleJoin} disabled={disabled} className="w-full rounded-lg bg-emerald-500 hover:bg-emerald-600 disabled:opacity-50 disabled:cursor-not-allowed py-3 font-medium transition">
          Join Lobby
        </button>
